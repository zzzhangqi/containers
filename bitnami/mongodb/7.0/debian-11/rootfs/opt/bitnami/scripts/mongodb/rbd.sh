...
if is_dir_empty "${MONGODB_DATA_DIR}/db"; then
  info "Data dir empty, checking if the replica set already exists"
    current_primary=$(mongosh admin --host "${SERVICE_NAME}-0.${SERVICE_NAME}.${NAMESPACE}.svc.cluster.local:27017,${SERVICE_NAME}-1.${SERVICE_NAME}.${NAMESPACE}.svc.cluster.local:27017" --authenticationDatabase admin -u $MONGODB_ROOT_USER -p $MONGODB_ROOT_PASSWORD --eval 'db.runCommand("ismaster")' | awk -F\' '/primary/ {print $2}')
...
elif [[ "$POD_NAME" = "${SERVICE_NAME}-0" ]]; then
